...
export default function ProjectModalPro({ project, onSave, onDelete, onClose }) {
  const [form, setForm] = useState(project ? structuredClone(project) : {
    name: '', client: '', total: 0, notes: '', status: 'active', dueDate: '', tasks: [], payments: [],
  });
  ...
  return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <section className="project-modal-pro">
      <header className="project-modal-pro-head">
        ...
        <div><span>{project ? 'AJUSTEMOS EL PROYECTO' : 'NUEVO PROYECTO'}</span><h2>{project ? `Vamos a actualizar ${project.name}.` : '¿Qué vamos a construir?'}</h2><p>{project ? 'Cambia solo lo que necesites. El resto se mantiene sincronizado.' : 'Dame lo esencial y yo lo organizo dentro de tu workspace.'}</p></div>
      ...
      {project && <div className="project-modal-context">
        ...
        <span><small>Entrega</small><b>{project.dueDate || 'Sin fecha'}</b></span>
      </div>}
      ...
      <form className="project-modal-form" onSubmit={(event) => { event.preventDefault(); onSave(form); }}>
        ...
        <div className="project-form-block">
          <div className="project-form-block-title"><span>02</span><div><strong>Negocio y entrega</strong><small>Lo que alimenta tu caja y tus alertas.</small></div></div>
