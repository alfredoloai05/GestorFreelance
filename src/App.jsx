...
const AUTH_KEY = 'gestor_freelance_auth_v1';
const TEMP_USER = 'alfredoloai';
const TEMP_PASS = 'Ruben2105.';
...
function Login({ onLogin }) {
  ...
  const submit = (event) => {
    ...
    if (user.trim() === TEMP_USER && password === TEMP_PASS) {
  ...
  return <div className="login-page"><div className="login-orbit orbit-one"/><div className="login-orbit orbit-two"/>
    <section className="login-card">
      ...
      <div className="login-copy"><span className="mini-label"><Icon name="spark" size={15}/> WORKSPACE PRIVADO</span><h1>Todo tu trabajo,<br/><span>en un solo lugar.</span></h1><p>Proyectos, equipo y números claros para que puedas enfocarte en construir.</p></div>
      <form onSubmit={submit} className="login-form"><label>Usuario<input value={user} onChange={(e) => { setUser(e.target.value); setError(false); }} autoFocus placeholder="Tu usuario"/></label><label>Contraseña<input value={password} onChange={(e) => { setPassword(e.target.value); setError(false); }} type="password" placeholder="••••••••••"/></label>{error && <div className="login-error">Usuario o contraseña incorrectos.</div>}<button className="btn btn-primary login-button">Entrar <Icon name="arrow"/></button></form>
      <p className="login-foot">Acceso temporal · Supabase Auth se conectará en la siguiente fase.</p>
    </section>
  </div>;
}

function Sidebar({ data, selectedProjectId, section, onDashboard, onProject, onTeam, onNewProject, onBackup, onRestore, onLogout, mobileOpen, closeMobile }) {
  ...
  return <>{mobileOpen && <button className="mobile-overlay" onClick={closeMobile} aria-label="Cerrar menú"/>}<aside className={`sidebar ${mobileOpen ? 'mobile-open' : ''}`}>
    <div className="sidebar-top"><button className="brand-button" onClick={() => { onDashboard(); closeMobile(); }}><span className="brand-mark">VX</span><span><strong>Vencodex</strong><small>Workspace</small></span></button><button className="mobile-close" onClick={closeMobile}><Icon name="x"/></button></div>
    ...
    <div className="sidebar-spacer"/><div className="storage-card"><div><span className="storage-icon"><Icon name="spark" size={16}/></span><strong>Modo local</strong></div><p>Datos de prueba en este navegador. Preparado para Supabase.</p></div>
    <div className="sidebar-tools"><button onClick={onBackup}><Icon name="download" size={16}/> Respaldar</button><button onClick={() => fileRef.current?.click()}><Icon name="upload" size={16}/> Restaurar</button><input ref={fileRef} type="file" accept=".json" hidden onChange={(e) => onRestore(e.target.files?.[0])}/><button onClick={onLogout}><Icon name="logout" size={16}/> Cerrar sesión</button></div>
  </aside></>;
}

function Header({ title, subtitle, onMenu, onNewProject }) {
  return <header className="app-header"><div className="header-title"><button className="menu-button" onClick={onMenu}><Icon name="menu"/></button><div><span>{subtitle}</span><h1>{title}</h1></div></div><div className="header-actions"><div className="header-search"><Icon name="search" size={16}/><span>Buscar</span><kbd>⌘ K</kbd></div><button className="btn btn-primary" onClick={onNewProject}><Icon name="plus" size={16}/> Proyecto</button><div className="avatar">AL</div></div></header>;
}
...
function Dashboard({ data, onProject, onNewProject }) {
  ...
  const focusTasks = useMemo(() => data.projects.flatMap((project) => (project.tasks || []).filter((task) => task.status !== 'done').map((task) => ({ ...task, projectName: project.name, projectId: project.id }))).sort((a,b) => Number(b.effort || 0) - Number(a.effort || 0)).slice(0,6), [data]);
  return <div className="page dashboard-page">
    <section className="welcome-card"><div><span className="mini-label"><Icon name="spark" size={15}/> PANEL DE CONTROL</span><h2>Buenas, Alfredo.<br/><span>Esto es lo que está pasando.</span></h2><p>Tienes {data.projects.length} proyectos activos y {data.projects.reduce((s,p) => s + p.tasks.filter(t => t.status !== 'done').length, 0)} tareas todavía abiertas.</p></div><div className="welcome-progress"><div className="ring" style={{ '--progress': `${summary.weighted * 3.6}deg` }}><span><b>{summary.weighted}%</b><small>avance global</small></span></div></div></section>
    ...
    <section className="content-grid dashboard-grid"><div className="panel project-panel"><div className="panel-head"><div><span className="section-kicker">PORTAFOLIO</span><h3>Proyectos activos</h3></div><button className="text-action" onClick={onNewProject}>Nuevo proyecto <Icon name="plus" size={15}/></button></div><div className="project-cards">{data.projects.map((project,index) => { const m = getProjectMetrics(project); return <button key={project.id} className="project-card" onClick={() => onProject(project.id)}><div className="project-card-top"><span className={`project-index p${index % 4}`}>{String(index+1).padStart(2,'0')}</span><Icon name="arrow"/></div><div className="project-card-copy"><span>{project.client || 'Proyecto interno'}</span><h4>{project.name}</h4><p>{project.notes || 'Sin notas todavía.'}</p></div><div className="project-card-progress"><div><span>Avance</span><strong>{m.progress}%</strong></div><ProgressBar value={m.progress} compact/></div><div className="project-card-foot"><span>{m.done}/{m.totalTasks} tareas</span><span>{money(m.due)} por cobrar</span></div></button>; })}</div></div>
      <aside className="panel focus-panel"><div className="panel-head"><div><span className="section-kicker">PRIORIDAD</span><h3>En foco</h3></div><span className="muted-count">{focusTasks.length}</span></div><div className="focus-list">{focusTasks.map((task) => <button key={`${task.projectId}-${task.id}`} onClick={() => onProject(task.projectId)}><span className="focus-check"/><span className="focus-copy"><b>{task.title}</b><small>{task.projectName} · {task.area}</small></span><span className="effort-badge">{task.effort}</span></button>)}</div></aside>
    </section>
  </div>;
}
...
function TasksView({ project, onEditTask, onNewTask, onImport }) {
  const [query,setQuery] = useState(''); const [status,setStatus] = useState('all'); const [area,setArea] = useState('all'); const fileRef = useRef(null);
  const areas = useMemo(() => [...new Set(project.tasks.map((task) => task.area || 'General'))].sort(), [project.tasks]);
  const filtered = useMemo(() => project.tasks.filter((task) => (!query || `${task.title} ${task.description} ${task.assignee}`.toLowerCase().includes(query.toLowerCase())) && (status === 'all' || task.status === status) && (area === 'all' || (task.area || 'General') === area)), [project.tasks,query,status,area]);
...
function PaymentsView({ project, onNewPayment }) {
  ...
  return <div className="payments-view"><section className="metrics-grid project-metrics"><MetricCard label="Ingresos registrados" value={money(metrics.received)} detail={`${money(metrics.due)} por cobrar`} icon="wallet" tone="accent"/><MetricCard label="Egresos registrados" value={money(metrics.paid)} detail={`${money(metrics.payable)} por pagar`} icon="users"/><MetricCard label="Balance actual" value={money(metrics.received-metrics.paid)} detail="Ingresos menos egresos" icon="grid"/><MetricCard label="Valor contratado" value={money(project.total)} detail={project.client || 'Proyecto'} icon="folder"/></section><section className="panel payment-panel"><div className="panel-head"><div><span className="section-kicker">MOVIMIENTOS</span><h3>Historial de pagos</h3></div><button className="btn btn-primary" onClick={onNewPayment}><Icon name="plus" size={15}/> Registrar pago</button></div>{payments.length ? <div className="payment-list">{payments.map((payment)=><div key={payment.id} className="payment-row"><span className={`payment-type ${payment.type}`}>{payment.type==='income'?'↙':'↗'}</span><div><b>{payment.party || (payment.type==='income'?'Ingreso':'Egreso')}</b><small>{payment.note || payment.date || 'Sin nota'}</small></div><span className={payment.type==='income'?'positive':'negative'}>{payment.type==='income'?'+':'-'}{money(payment.amount)}</span></div>)}</div> : <div className="empty-state compact-empty"><span><Icon name="wallet"/></span><h4>Todavía no hay movimientos</h4><p>Registra cobros de clientes o pagos a colaboradores.</p><button className="btn btn-ghost" onClick={onNewPayment}>Registrar el primero</button></div>}</section></div>;
}

function ProjectWorkspace({ project, tab, setTab, onEditProject, onNewTask, onEditTask, onImport, onNewPayment }) {
  const metrics = getProjectMetrics(project);
  return <div className="page project-page"><section className="project-hero"><div className="project-hero-main"><span className="project-breadcrumb">Proyectos <Icon name="chevron" size={13}/> {project.name}</span><div className="project-title-row"><div><span className="project-client">{project.client || 'Proyecto Vencodex'}</span><h2>{project.name}</h2><p>{project.notes || 'Sin descripción todavía.'}</p></div><button className="btn btn-ghost" onClick={onEditProject}>Editar proyecto</button></div></div><div className="project-score"><span>Avance total</span><strong>{metrics.progress}%</strong><ProgressBar value={metrics.progress}/><small>{metrics.done} de {metrics.totalTasks} tareas completadas</small></div></section><nav className="project-tabs">{[['overview','Resumen'],['tasks','Tareas'],['payments','Pagos']].map(([key,label])=><button key={key} className={tab===key?'active':''} onClick={()=>setTab(key)}>{label}{key==='tasks'&&<span>{metrics.totalTasks}</span>}</button>)}</nav>{tab==='overview'&&<ProjectOverview project={project}/>} {tab==='tasks'&&<TasksView project={project} onEditTask={onEditTask} onNewTask={onNewTask} onImport={onImport}/>} {tab==='payments'&&<PaymentsView project={project} onNewPayment={onNewPayment}/>}</div>;
}

function TeamView({ data, onAdd, onRemove }) {
  return <div className="page team-page"><section className="panel team-hero"><div><span className="section-kicker">EQUIPO</span><h2>Las personas detrás del trabajo.</h2><p>Agrega colaboradores una sola vez y asígnalos después a cualquier tarea.</p></div><button className="btn btn-primary" onClick={onAdd}><Icon name="plus" size={15}/> Colaborador</button></section><section className="team-grid">{data.collaborators.map((person,index)=>{const tasks=data.projects.flatMap(p=>p.tasks).filter(t=>t.assignee===person);const open=tasks.filter(t=>t.status!=='done').length;return <article key={person} className="team-card"><div className={`team-avatar a${index%4}`}>{person.split(' ').map(v=>v[0]).slice(0,2).join('').toUpperCase()}</div><div className="team-card-copy"><h3>{person}</h3><p>{index===0?'Administrador · Vencodex':'Colaborador'}</p></div><div className="team-stats"><span><b>{tasks.length}</b> tareas</span><span><b>{open}</b> abiertas</span></div>{index!==0&&<button className="remove-link" onClick={()=>onRemove(person)}>Quitar</button>}</article>;})}</section></div>;
}
...
function ProjectModal({ project,onSave,onClose }) { const [form,setForm]=useState(project?clone(project):{name:'',client:'',total:0,notes:'',status:'active'});const set=(k,v)=>setForm(p=>({...p,[k]:v}));return <Modal title={project?'Editar proyecto':'Nuevo proyecto'} eyebrow="PROYECTO" onClose={onClose}><form className="form-stack" onSubmit={(e)=>{e.preventDefault();onSave(form);}}><label>Nombre del proyecto<input required value={form.name} onChange={(e)=>set('name',e.target.value)} placeholder="Ej. RIOSAC"/></label><div className="form-grid"><label>Cliente<input value={form.client||''} onChange={(e)=>set('client',e.target.value)} placeholder="Nombre del cliente"/></label><label>Valor total<input type="number" min="0" step="0.01" value={form.total||0} onChange={(e)=>set('total',Number(e.target.value))}/></label></div><label>Descripción<textarea rows="4" value={form.notes||''} onChange={(e)=>set('notes',e.target.value)} placeholder="Contexto breve del proyecto"/></label><div className="modal-actions"><button type="button" className="btn btn-ghost" onClick={onClose}>Cancelar</button><button className="btn btn-primary">Guardar proyecto</button></div></form></Modal>; }
function TaskModal({ task,collaborators,onSave,onDelete,onClose }) { const [form,setForm]=useState(task?clone(task):{title:'',description:'',area:'General',status:'pending',progress:0,effort:3,assignee:'',dueDate:'',cost:0,notes:''});const set=(k,v)=>setForm(p=>({...p,[k]:v}));const changeStatus=(value)=>setForm(p=>({...p,status:value,progress:value==='done'?100:p.status==='done'&&p.progress===100?0:p.progress}));return <Modal title={task?'Editar tarea':'Nueva tarea'} eyebrow="TAREA" onClose={onClose} wide><form className="form-stack" onSubmit={(e)=>{e.preventDefault();onSave(form);}}><label>Título<input required value={form.title} onChange={(e)=>set('title',e.target.value)} placeholder="¿Qué hay que hacer?"/></label><label>Descripción<textarea rows="3" value={form.description||''} onChange={(e)=>set('description',e.target.value)} placeholder="Resultado esperado o contexto"/></label><div className="form-grid three"><label>Área<input value={form.area||''} onChange={(e)=>set('area',e.target.value)} placeholder="Backend"/></label><label>Responsable<select value={form.assignee||''} onChange={(e)=>set('assignee',e.target.value)}><option value="">Sin asignar</option>{collaborators.map(p=><option key={p}>{p}</option>)}</select></label><label>Esfuerzo<select value={form.effort||3} onChange={(e)=>set('effort',Number(e.target.value))}>{[1,2,3,5,8].map(v=><option key={v} value={v}>{v} puntos</option>)}</select></label></div><div className="form-grid three"><label>Estado<select value={form.status} onChange={(e)=>changeStatus(e.target.value)}>{Object.entries(statusMeta).map(([key,meta])=><option key={key} value={key}>{meta.label}</option>)}</select></label><label>Avance<select value={form.progress||0} onChange={(e)=>set('progress',Number(e.target.value))}>{[0,25,50,75,100].map(v=><option key={v} value={v}>{v}%</option>)}</select></label><label>Fecha límite<input type="date" value={form.dueDate||''} onChange={(e)=>set('dueDate',e.target.value)}/></label></div><div className="form-grid"><label>Costo acordado<input type="number" min="0" step="0.01" value={form.cost||0} onChange={(e)=>set('cost',Number(e.target.value))}/></label><label>Notas internas<input value={form.notes||''} onChange={(e)=>set('notes',e.target.value)} placeholder="Opcional"/></label></div><div className="modal-actions split">{task?<button type="button" className="danger-link" onClick={onDelete}>Eliminar tarea</button>:<span/>}<div><button type="button" className="btn btn-ghost" onClick={onClose}>Cancelar</button><button className="btn btn-primary">Guardar cambios</button></div></div></form></Modal>; }
function PaymentModal({ project,onSave,onClose }) { const [form,setForm]=useState({type:'income',amount:'',date:new Date().toISOString().slice(0,10),party:'',taskId:'',note:''});const set=(k,v)=>setForm(p=>({...p,[k]:v}));return <Modal title="Registrar movimiento" eyebrow="FINANZAS" onClose={onClose}><form className="form-stack" onSubmit={(e)=>{e.preventDefault();onSave({...form,amount:Number(form.amount)});}}><div className="form-grid"><label>Tipo<select value={form.type} onChange={(e)=>set('type',e.target.value)}><option value="income">Me pagaron</option><option value="expense">Yo pagué</option></select></label><label>Monto<input required type="number" min="0.01" step="0.01" value={form.amount} onChange={(e)=>set('amount',e.target.value)} placeholder="0.00"/></label></div><div className="form-grid"><label>Fecha<input type="date" value={form.date} onChange={(e)=>set('date',e.target.value)}/></label><label>Persona / concepto<input value={form.party} onChange={(e)=>set('party',e.target.value)} placeholder="Cliente, developer, hosting..."/></label></div>{form.type==='expense'&&<label>Relacionar con tarea<select value={form.taskId} onChange={(e)=>set('taskId',e.target.value)}><option value="">Sin tarea específica</option>{project.tasks.map(t=><option key={t.id} value={t.id}>{t.title}</option>)}</select></label>}<label>Nota<textarea rows="3" value={form.note} onChange={(e)=>set('note',e.target.value)} placeholder="Detalle opcional"/></label><div className="modal-actions"><button type="button" className="btn btn-ghost" onClick={onClose}>Cancelar</button><button className="btn btn-primary">Registrar movimiento</button></div></form></Modal>; }
...
export default function App() {
  const [authenticated,setAuthenticated]=useState(localStorage.getItem(AUTH_KEY)==='ok'); const [data,setData]=useState(null); const [section,setSection]=useState('dashboard'); const [selectedProjectId,setSelectedProjectId]=useState(null); const [projectTab,setProjectTab]=useState('overview'); const [modal,setModal]=useState(null); const [mobileOpen,setMobileOpen]=useState(false); const hydrated=useRef(false);
  useEffect(()=>{dataService.load().then((loaded)=>{setData(loaded);hydrated.current=true;});},[]);
  useEffect(()=>{if(hydrated.current&&data)dataService.save(data);},[data]);
  if(!authenticated)return <Login onLogin={()=>setAuthenticated(true)}/>; if(!data)return <div className="loading-screen"><span className="brand-mark">VX</span><p>Cargando workspace…</p></div>;
  ...
  const savePayment=(form)=>{if(!selectedProject)return;setData(prev=>({...prev,projects:prev.projects.map(project=>project.id===selectedProject.id?{...project,payments:[...(project.payments||[]),{...form,id:uid('payment')}]}:project)}));setModal(null);};
  const importTasks=(file)=>{if(!file||!selectedProject)return;const reader=new FileReader();reader.onload=()=>{const tasks=String(reader.result||'').split(/\r?\n/).map(line=>line.trim()).filter(Boolean).map((line)=>{const [title,description='',cost='0',area='General',effort='3']=line.split('|').map(v=>v.trim());return{id:uid('task'),title,description,cost:Number(cost)||0,area:area||'General',effort:Number(effort)||3,status:'pending',progress:0,assignee:'',dueDate:'',notes:''};}).filter(task=>task.title);if(!tasks.length)return;setData(prev=>({...prev,projects:prev.projects.map(project=>project.id===selectedProject.id?{...project,tasks:[...project.tasks,...tasks]}:project)}));};reader.readAsText(file);};
  const backup=()=>{const blob=new Blob([JSON.stringify(data,null,2)],{type:'application/json'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=`vencodex-workspace-${new Date().toISOString().slice(0,10)}.json`;a.click();URL.revokeObjectURL(url);};
  const restore=(file)=>{if(!file)return;const reader=new FileReader();reader.onload=()=>{try{const parsed=JSON.parse(String(reader.result));if(!Array.isArray(parsed.projects))throw new Error('Formato inválido');setData(parsed);openDashboard();}catch{window.alert('El archivo no parece ser un respaldo válido.');}};reader.readAsText(file);};
  ...
  return <div className="app-shell"><Sidebar data={data} selectedProjectId={selectedProjectId} section={section} onDashboard={openDashboard} onProject={openProject} onTeam={openTeam} onNewProject={()=>setModal({type:'project'})} onBackup={backup} onRestore={restore} onLogout={()=>{localStorage.removeItem(AUTH_KEY);setAuthenticated(false);}} mobileOpen={mobileOpen} closeMobile={()=>setMobileOpen(false)}/><main className="main-area"><Header title={headerTitle} subtitle={headerSubtitle} onMenu={()=>setMobileOpen(true)} onNewProject={()=>setModal({type:'project'})}/>{section==='dashboard'&&<Dashboard data={data} onProject={openProject} onNewProject={()=>setModal({type:'project'})}/>} {section==='team'&&<TeamView data={data} onAdd={()=>setModal({type:'collaborator'})} onRemove={removeCollaborator}/>} {section==='project'&&selectedProject&&<ProjectWorkspace project={selectedProject} tab={projectTab} setTab={setProjectTab} onEditProject={()=>setModal({type:'project',project:selectedProject})} onNewTask={()=>setModal({type:'task'})} onEditTask={(task)=>setModal({type:'task',task})} onImport={importTasks} onNewPayment={()=>setModal({type:'payment'})}/>}</main>{modal?.type==='project'&&<ProjectModal project={modal.project} onSave={saveProject} onClose={()=>setModal(null)}/>} {modal?.type==='task'&&<TaskModal task={modal.task} collaborators={data.collaborators} onSave={saveTask} onDelete={()=>deleteTask(modal.task.id)} onClose={()=>setModal(null)}/>} {modal?.type==='payment'&&selectedProject&&<PaymentModal project={selectedProject} onSave={savePayment} onClose={()=>setModal(null)}/>} {modal?.type==='collaborator'&&<CollaboratorModal onSave={addCollaborator} onClose={()=>setModal(null)}/>}</div>;
}
